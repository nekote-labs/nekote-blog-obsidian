...
| 8 | リボン・ノートヘッダー・メニューのアイコン表示と、モバイルでの表示位置 | macOS・iOS・Android | 公開対象ノートを開き、ヘッダーの反映ボタンの表示・非表示（公開対象外ノートで消えること）とリボン・ヘッダー・右クリックの各メニュー（未接続では反映項目がラベルに置き換わり、画像選択とFront Matter挿入は残ること）を目視する | アイコン名 `cat`・`upload`・`settings`・`image` が同梱lucideに存在する前提。無くても機能は動くがアイコンが空になる（`src/main.ts`） |
...
| 11 | プロパティ欄直下の画像選択ボタン行のDOM注入 | macOS・iOS・Android | 公開対象ノートをLive Previewとリーディングビューで開き、プロパティ欄の直下に「サムネイルを選択」「カバー画像を選択」が出ること、プロパティ編集・モード切替・ノート切替の後も残ること、公開対象外ノートでは出ないことを目視する | 内部DOMのクラス名（`.metadata-container`・`.cm-content`・`.is-collapsed`）に依存する。構造が変われば出なくなるだけで、同期機能には影響しない（`src/ui/properties-actions.ts` の `PropertiesActions`） |
